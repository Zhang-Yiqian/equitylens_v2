import type { InflectionAnalysis, CrossValidationAnalysis, FinancialSnapshot, Report } from '@equitylens/core';
import { formatFinancialValue, DIMENSIONS, CROSS_DIMENSIONS } from '@equitylens/core';

export function renderMarkdownReport(
  analysis: InflectionAnalysis,
  financial: FinancialSnapshot,
  validationSummary?: { verifiedCount: number; totalEvidence: number; failedCount: number },
): string {
  const lines: string[] = [];
  const now = new Date();

  // Header
  lines.push(`# ${analysis.ticker} Inflection Analysis Report`);
  lines.push(`**Period**: FY${analysis.year} Q${analysis.quarter} | **Generated**: ${now.toISOString().split('T')[0]} | **Model**: ${analysis.modelId}`);
  lines.push(`**Prompt Version**: ${analysis.promptVersion}`);
  lines.push('');

  // Verdict
  lines.push('---');
  lines.push('## Verdict');
  lines.push('');
  const verdictEmoji = analysis.verdict === 'Conviction Buy' ? '🟢' : analysis.verdict === 'Watch' ? '🟡' : '🔴';
  lines.push(`### ${verdictEmoji} ${analysis.verdict} (Confidence: ${analysis.verdictConfidence}/100)`);
  lines.push('');
  lines.push(`> ${analysis.thesisSummary}`);
  lines.push('');

  // Token Usage & Cost
  const modelCost = estimateCost(analysis.tokenUsage);
  lines.push(`*Token usage: ${analysis.tokenUsage.promptTokens.toLocaleString()} input + ${analysis.tokenUsage.completionTokens.toLocaleString()} output = ${analysis.tokenUsage.totalTokens.toLocaleString()} total (~$${modelCost})*`);
  lines.push('');

  // Financial Snapshot
  lines.push('---');
  lines.push('## Financial Snapshot (Hard Truth)');
  lines.push('');
  lines.push('| Metric | Value |');
  lines.push('|--------|-------|');
  lines.push(`| Revenue | ${formatFinancialValue(financial.revenue)} |`);
  lines.push(`| Net Income | ${formatFinancialValue(financial.netIncome)} |`);
  lines.push(`| Gross Margin | ${financial.grossMarginPct !== null ? financial.grossMarginPct.toFixed(1) + '%' : '缺失'} |`);
  lines.push(`| Operating Cash Flow | ${formatFinancialValue(financial.operatingCashFlow)} |`);
  lines.push(`| Free Cash Flow | ${formatFinancialValue(financial.freeCashFlow)} |`);
  lines.push(`| FCF Margin | ${financial.fcfMarginPct !== null ? financial.fcfMarginPct.toFixed(1) + '%' : '缺失'} |`);
  lines.push(`| R&D Expense | ${formatFinancialValue(financial.rdExpense)} |`);
  lines.push(`| EPS (Diluted) | ${financial.eps !== null ? '$' + financial.eps.toFixed(2) : '缺失'} |`);
  lines.push(`| Market Cap | ${formatFinancialValue(financial.marketCap)} |`);
  lines.push(`| P/E Ratio | ${financial.peRatio !== null ? financial.peRatio.toFixed(1) + 'x' : '缺失'} |`);
  lines.push(`| Deferred Revenue | ${formatFinancialValue(financial.deferredRevenue)} |`);
  lines.push('');

  // 12 Dimensions
  lines.push('---');
  lines.push('## 12-Dimension Inflection Analysis');
  lines.push('');

  const categories = ['A', 'B', 'C', 'D'] as const;
  const categoryNames: Record<string, string> = {
    A: 'Financial Leading Indicators (财务先行指标)',
    B: 'Hardware/Semiconductor (硬件/半导体纵深)',
    C: 'Software/SaaS (软件/SaaS纵深)',
    D: 'Sentiment & Game Theory (情绪与博弈)',
  };

  for (const cat of categories) {
    const catDims = analysis.dimensions.filter(d => d.category === cat);
    if (catDims.length === 0) continue;

    lines.push(`### [${cat}] ${categoryNames[cat]}`);
    lines.push('');

    for (const dim of catDims) {
      const signalIcon = dim.signal === 'bullish' ? '📈'
        : dim.signal === 'bearish' ? '📉'
        : dim.signal === 'skipped' ? '⏭️'
        : '➡️';

      const dimDef = DIMENSIONS.find(d => d.id === dim.id);
      const nameZh = dimDef?.nameZh || '';

      lines.push(`#### ${dim.id}: ${dim.name} (${nameZh})`);
      lines.push(`**Signal**: ${signalIcon} ${dim.signal.toUpperCase()} | **Confidence**: ${dim.confidence}/100`);
      lines.push('');
      lines.push(dim.summary);
      lines.push('');

      if (dim.evidence.length > 0) {
        lines.push('**Evidence:**');
        for (const ev of dim.evidence) {
          lines.push(`- > "${ev.quote}" *(${ev.source})*`);
        }
        lines.push('');
      }
    }
  }

  // Signal Summary Table
  lines.push('### Signal Summary');
  lines.push('');
  lines.push('| Dimension | Signal | Confidence |');
  lines.push('|-----------|--------|------------|');
  for (const dim of analysis.dimensions) {
    const icon = dim.signal === 'bullish' ? '📈' : dim.signal === 'bearish' ? '📉' : dim.signal === 'skipped' ? '⏭️' : '➡️';
    lines.push(`| ${dim.id}: ${dim.name} | ${icon} ${dim.signal} | ${dim.confidence} |`);
  }
  lines.push('');

  // Catalysts
  lines.push('---');
  lines.push('## Catalysts');
  lines.push('');
  for (const cat of analysis.catalysts) {
    const probIcon = cat.probability === 'high' ? '🔥' : cat.probability === 'medium' ? '⚡' : '💡';
    lines.push(`### ${probIcon} ${cat.description}`);
    lines.push(`**Timeline**: ${cat.timeline} | **Probability**: ${cat.probability}`);
    lines.push('');
    if (cat.evidence.length > 0) {
      for (const ev of cat.evidence) {
        lines.push(`> "${ev.quote}" *(${ev.source})*`);
      }
      lines.push('');
    }
  }

  // Risks
  lines.push('---');
  lines.push('## Risks');
  lines.push('');
  for (const risk of analysis.risks) {
    const sevIcon = risk.severity === 'high' ? '🚨' : risk.severity === 'medium' ? '⚠️' : 'ℹ️';
    lines.push(`### ${sevIcon} ${risk.description}`);
    lines.push(`**Severity**: ${risk.severity}`);
    lines.push('');
    if (risk.evidence.length > 0) {
      for (const ev of risk.evidence) {
        lines.push(`> "${ev.quote}" *(${ev.source})*`);
      }
      lines.push('');
    }
  }

  // Tracking Metrics
  lines.push('---');
  lines.push('## Tracking Metrics (Next Quarter Checklist)');
  lines.push('');
  lines.push('| Metric | Current | Target | Check Date |');
  lines.push('|--------|---------|--------|------------|');
  for (const tm of analysis.trackingMetrics) {
    lines.push(`| ${tm.metric} | ${tm.currentValue} | ${tm.targetValue} | ${tm.nextCheckDate} |`);
  }
  lines.push('');

  // Validation
  if (validationSummary) {
    lines.push('---');
    lines.push('## Evidence Validation');
    lines.push('');
    const pct = validationSummary.totalEvidence > 0
      ? ((validationSummary.verifiedCount / validationSummary.totalEvidence) * 100).toFixed(0)
      : '0';
    lines.push(`- **Total evidence citations**: ${validationSummary.totalEvidence}`);
    lines.push(`- **Verified**: ${validationSummary.verifiedCount} (${pct}%)`);
    lines.push(`- **Unverified**: ${validationSummary.failedCount}`);
    lines.push('');
  }

  // Footer
  lines.push('---');
  lines.push(`*Report generated by EquityLens v2 | ${now.toISOString()}*`);

  return lines.join('\n');
}

/**
 * Render a cross-validation analysis report (三段式 + 8 维度).
 */
export function renderCrossValidationReport(
  analysis: CrossValidationAnalysis,
  financial: FinancialSnapshot,
  validationSummary?: { verifiedCount: number; totalEvidence: number; failedCount: number },
): string {
  const lines: string[] = [];
  const now = new Date();

  // Header
  lines.push(`# ${analysis.ticker} 交叉验证深度分析报告`);
  lines.push(`**报告期**: FY${analysis.year} Q${analysis.quarter} | **生成时间**: ${now.toISOString().split('T')[0]} | **模型**: ${analysis.modelId}`);
  lines.push(`**Prompt版本**: ${analysis.promptVersion}`);
  lines.push('');

  // Verdict badge
  lines.push('---');
  const verdictEmoji = analysis.verdict === 'Buy' ? '🟢' : analysis.verdict === 'Watch' ? '🟡' : '🔴';
  lines.push(`## ${verdictEmoji} 评级: ${analysis.verdict}`);
  lines.push('');

  // Token usage
  const modelCost = estimateCost(analysis.tokenUsage);
  lines.push(`*Token 用量: ${analysis.tokenUsage.promptTokens.toLocaleString()} 输入 + ${analysis.tokenUsage.completionTokens.toLocaleString()} 输出 = ${analysis.tokenUsage.totalTokens.toLocaleString()} 合计 (~$${modelCost})*`);
  lines.push('');

  // === Section 1: 综合结论 ===
  lines.push('---');
  lines.push('## 一、综合结论');
  lines.push('');
  lines.push(analysis.conclusion);
  lines.push('');

  // === Section 2: 竞争格局分析 ===
  lines.push('---');
  lines.push('## 二、竞争格局分析（基于10-K）');
  lines.push('');
  lines.push(analysis.landscapeAnalysis);
  lines.push('');

  // === Section 3: 核心风险提示 ===
  lines.push('---');
  lines.push('## 三、核心风险提示');
  lines.push('');
  lines.push(analysis.riskWarning);
  lines.push('');

  // === Financial Snapshot ===
  lines.push('---');
  lines.push('## 财务快照（Hard Truth）');
  lines.push('');
  lines.push('| 指标 | 数值 |');
  lines.push('|------|------|');
  lines.push(`| 营收 | ${formatFinancialValue(financial.revenue)} |`);
  lines.push(`| 净利润 | ${formatFinancialValue(financial.netIncome)} |`);
  lines.push(`| 毛利率 | ${financial.grossMarginPct !== null ? financial.grossMarginPct.toFixed(1) + '%' : '缺失'} |`);
  lines.push(`| 经营现金流 | ${formatFinancialValue(financial.operatingCashFlow)} |`);
  lines.push(`| 自由现金流 | ${formatFinancialValue(financial.freeCashFlow)} |`);
  lines.push(`| FCF Margin | ${financial.fcfMarginPct !== null ? financial.fcfMarginPct.toFixed(1) + '%' : '缺失'} |`);
  lines.push(`| R&D支出 | ${formatFinancialValue(financial.rdExpense)} |`);
  lines.push(`| EPS（稀释） | ${financial.eps !== null ? '$' + financial.eps.toFixed(2) : '缺失'} |`);
  lines.push(`| 市值 | ${formatFinancialValue(financial.marketCap)} |`);
  lines.push(`| 市盈率 | ${financial.peRatio !== null ? financial.peRatio.toFixed(1) + 'x' : '缺失'} |`);
  lines.push('');

  // === 8 Dimensions ===
  lines.push('---');
  lines.push('## 8维度交叉验证矩阵');
  lines.push('');

  const catMap: Record<string, { name: string; dims: typeof analysis.dimensions }> = {
    A: { name: '[A] 10-K 竞争格局基线', dims: [] },
    B: { name: '[B] 财务异动侦测', dims: [] },
    C: { name: '[C] 新闻事件验证', dims: [] },
  };

  for (const dim of analysis.dimensions) {
    const cat = dim.category as 'A' | 'B' | 'C';
    if (catMap[cat]) catMap[cat].dims.push(dim);
  }

  for (const [cat, { name, dims }] of Object.entries(catMap)) {
    if (dims.length === 0) continue;
    lines.push(`### ${name}`);
    lines.push('');

    for (const dim of dims) {
      const signalIcon = dim.signal === 'bullish' ? '📈'
        : dim.signal === 'bearish' ? '📉'
        : dim.signal === 'skipped' ? '⏭️'
        : '➡️';

      const dimDef = CROSS_DIMENSIONS.find(d => d.id === dim.id);
      const nameZh = dimDef?.nameZh ?? dim.name;

      lines.push(`#### ${dim.id}: ${nameZh}`);
      lines.push(`**信号**: ${signalIcon} ${dim.signal.toUpperCase()} | **置信度**: ${dim.confidence}/100`);
      lines.push('');
      lines.push(dim.summary);
      lines.push('');

      if (dim.evidence.length > 0) {
        lines.push('**证据来源:**');
        for (const ev of dim.evidence) {
          const srcLabel = ev.source === '10k' ? '10-K年报'
            : ev.source === 'news' ? '新闻'
            : ev.source === 'financial' ? '财务数据'
            : ev.source;
          lines.push(`- > "${ev.quote}" *(${srcLabel})*`);
          if (ev.context) lines.push(`  > ${ev.context}`);
        }
        lines.push('');
      }
    }
  }

  // Signal Summary Table
  lines.push('### 维度信号汇总');
  lines.push('');
  lines.push('| 维度 | 信号 | 置信度 |');
  lines.push('|------|------|--------|');
  for (const dim of analysis.dimensions) {
    const icon = dim.signal === 'bullish' ? '📈' : dim.signal === 'bearish' ? '📉' : dim.signal === 'skipped' ? '⏭️' : '➡️';
    const dimDef = CROSS_DIMENSIONS.find(d => d.id === dim.id);
    lines.push(`| ${dim.id}: ${dimDef?.nameZh ?? dim.name} | ${icon} ${dim.signal} | ${dim.confidence} |`);
  }
  lines.push('');

  // Catalysts
  if (analysis.catalysts.length > 0) {
    lines.push('---');
    lines.push('## 催化剂');
    lines.push('');
    for (const cat of analysis.catalysts) {
      const probIcon = cat.probability === 'high' ? '🔥' : cat.probability === 'medium' ? '⚡' : '💡';
      lines.push(`### ${probIcon} ${cat.description}`);
      lines.push(`**时间线**: ${cat.timeline} | **概率**: ${cat.probability}`);
      lines.push('');
      for (const ev of cat.evidence) {
        lines.push(`> "${ev.quote}" *(${ev.source})*`);
      }
      if (cat.evidence.length > 0) lines.push('');
    }
  }

  // Risks
  if (analysis.risks.length > 0) {
    lines.push('---');
    lines.push('## 风险清单');
    lines.push('');
    for (const risk of analysis.risks) {
      const sevIcon = risk.severity === 'high' ? '🚨' : risk.severity === 'medium' ? '⚠️' : 'ℹ️';
      lines.push(`### ${sevIcon} ${risk.description}`);
      lines.push(`**严重程度**: ${risk.severity}`);
      lines.push('');
      for (const ev of risk.evidence) {
        lines.push(`> "${ev.quote}" *(${ev.source})*`);
      }
      if (risk.evidence.length > 0) lines.push('');
    }
  }

  // Validation
  if (validationSummary) {
    lines.push('---');
    lines.push('## 证据验证');
    lines.push('');
    const pct = validationSummary.totalEvidence > 0
      ? ((validationSummary.verifiedCount / validationSummary.totalEvidence) * 100).toFixed(0)
      : '0';
    lines.push(`- **引用总数**: ${validationSummary.totalEvidence}`);
    lines.push(`- **已验证**: ${validationSummary.verifiedCount} (${pct}%)`);
    lines.push(`- **未验证**: ${validationSummary.failedCount}`);
    lines.push('');
  }

  lines.push('---');
  lines.push(`*报告由 EquityLens v2 生成 | ${now.toISOString()}*`);

  return lines.join('\n');
}

function estimateCost(usage: { promptTokens: number; completionTokens: number }): string {
  // Rough estimate using Gemini 2.5 Pro pricing
  const inputCost = (usage.promptTokens / 1_000_000) * 1.25;
  const outputCost = (usage.completionTokens / 1_000_000) * 10;
  return (inputCost + outputCost).toFixed(4);
}
